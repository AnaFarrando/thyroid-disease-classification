...
def main(args: argparse.Namespace) -> Optional[npt.ArrayLike]:
    if args.predict is None:
        ...
        pipeline = sklearn.pipeline.Pipeline([ # Create pipeline with scaler, polynomial features, and logistic regression
            ("poly", sklearn.preprocessing.PolynomialFeatures(include_bias=False)),
            ("scaler", sklearn.preprocessing.StandardScaler()),
            ("logr", sklearn.linear_model.LogisticRegression(max_iter=5000, random_state=args.seed))
        ])

        param_grid = { # Define parameter grid for grid search
        'poly__degree': [1, 2],
        'logr__C': [0.01, 0.1, 1, 10, 100],
        'logr__solver': ['lbfgs']
        }

        skf = sklearn.model_selection.StratifiedKFold(n_splits=5, shuffle=True, random_state=args.seed) # Define stratified k-fold cross-validator with 5 folds
        grid = sklearn.model_selection.GridSearchCV(pipeline, param_grid, cv=skf, scoring="accuracy", n_jobs=-1, verbose=0)
